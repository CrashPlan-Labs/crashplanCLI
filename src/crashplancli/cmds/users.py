...
def show_org(
    ...
    try:
        ...
        raise crashplancliError(f"Invalid org UID {org_uid}.")
...
def _get_legacy_user_id(sdk, username):
    if not username:
        # pycpg returns all users when passing `None` to `get_by_username()`.
...
def _get_role_id(sdk, role_name):
    try:
        ...
        raise crashplancliError(f"Role with name '{role_name}' not found.")
