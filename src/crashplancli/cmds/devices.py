...
from crashplancli.options import format_option
...
def rename(state, device_guid, new_device_name):
    """Rename a device within crashplan. Requires the device GUID to rename."""
...
def deactivate(state, device_guid, change_device_name, purge_date):
    """Deactivate a device within crashplan. Requires the device GUID to deactivate."""
...
def reactivate(state, device_guid):
    """Reactivate a device within crashplan. Requires the device GUID to reactivate."""
...
def _change_device_activation(sdk, device_guid, cmd_str):
    try:
        ...
        return device
    except exceptions.PycpgNotFoundError:
        raise crashplancliError(f"The device with GUID '{device_guid}' was not found.")
    except exceptions.PycpgForbiddenError:
        raise crashplancliError(
...
def _verify_guid_type(device_guid):
    ...
    try:
        ...
        raise crashplancliError("Not a valid GUID.")
...
def _change_device_name(sdk, guid, name):
    try:
        ...
        raise crashplancliError(f"The device with GUID '{guid}' was not found.")
...
def _device_info_keys_map():
    return {
        ...
        "productVersion": "CrashPlan Version",
